    ...
    <div className="animate-fade-up space-y-6 md:space-y-8">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="t-body text-muted-fg mt-3 max-w-xl">
            Edit system prompts, tools, and skills for each bot. Personas define how
      ...
      {bots && bots.length === 0 && (
        <div className="halftone-light py-24 text-center border border-border">
          ...
          <p className="t-headline text-muted-fg/60 mb-2">No configs</p>
          <p className="t-body text-muted-fg/70 mb-6">Create a config first, then edit its persona.</p>
          <Link
            to="/config"
            className="t-label text-fg hover:text-muted-fg transition-colors underline underline-offset-4"
          >
            Go to Configs
          </Link>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {bots.map((bot) => (
            <Card
              ...
              padding="none"
              onClick={() => navigate(`/persona/${bot.id}`)}
            >
              <div className="p-4 flex-1">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <span className="t-mono text-muted-fg truncate ml-3 shrink-0">
                    {bot.model_ref || "—"}
                  </span>
                </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Badge variant="info">
                    ...
                    {bot.skills_allowlist_json?.length ?? 0} skills
                  </Badge>
                </div>
              </div>
            </Card>
      ...
      <Modal open={modalOpen} onClose={() => { setModalOpen(false); setNewId(""); setNewDisplayName(""); setIdError(""); }} title="New Persona" size="sm">
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-3 pt-1">
            <Button variant="secondary" onClick={() => { setModalOpen(false); setNewId(""); setNewDisplayName(""); setIdError(""); }}>
              Cancel
            </Button>
